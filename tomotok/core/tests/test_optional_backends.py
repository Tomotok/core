...
class JaxBackendTestCase(unittest.TestCase):
    def test_jax_engines_solve_linear_system(self):
        from tomotok.core.inversions.jax import CholeskyJaxEngine, JaxEngine

        a = np.array([[4.0, 1.0], [1.0, 3.0]])
        b = np.array([1.0, 2.0])
        expected = np.linalg.solve(a, b)

        np.testing.assert_allclose(CholeskyJaxEngine().solve(a, b), expected)
        np.testing.assert_allclose(JaxEngine().solve(a, b), expected)
...
class SkSparseBackendTestCase(unittest.TestCase):
    def test_cholmod_engine_solves_linear_system(self):
        from tomotok.core.inversions.sksparse import CholmodEngine

        a = np.array([[4.0, 1.0], [1.0, 3.0]])
        b = np.array([1.0, 2.0])
        expected = np.linalg.solve(a, b)

        np.testing.assert_allclose(CholmodEngine().solve(a, b), expected)