...
A linear regularisation matrix is independent on intermediate solution in iterative process, a simplification compared to nonlinear regularisation.
This renders iterative approach used by the MFR algorithm unnecessary and the solution can be obtained in a single step.
It is used in the following form consisting of discretised derivation matrices :math:`\mathbf{B}_{i}` that can be weighted by coefficients :math:`c_i` to introduce anisotropy

.. math::
    \mathbf{H} = \sum\limits_i c_i \mathbf{B}^{T}_{i} \cdot \mathbf{B}_{i} \,.

Various numerical schemes can be used for computation of derivation matrices. The regularisation matrix computation is simplified compared to MFR and does not include the nonlinear term. This enables estimation of regularisation parameter directly from the decomposed matrices.
...
This matrix is then used as the matrix for investigation in eigenvalue problem. The diagonal matrix to be used in equation :eq:`series` is directly determined by solving generalised eigenvalue problem
...
The remaining matrices for use in series expansion :eq:`series` are then calculated as
